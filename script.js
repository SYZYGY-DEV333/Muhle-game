...
function initializeGame() {
    ...
    initializeArray();
    alert("Spieler 1 (Grün) dreht zuerst, gefolgt von Spieler 2 (rot).");
}
    ...
    if (isMillGreen || isMillRed) {
        ...
        if (positionMatrix[X][Y] != playerCode && (positionMatrix[X][Y] != 0)) {
            ...
            if (!checkMill(X, Y, ((isMillRed) ? 1 : 2)) || allArePartOfMill(((isMillRed) ? 1 : 2))) {
                ...
                if (playerCode == 1) {
                    redBlocks--;
                    document.getElementById("message").innerHTML = "Roter Block entfernt";
                } else {
                    document.getElementById("message").innerHTML = "Grüner Block entfernt";
                    greenBlocks--;
                }
                ...
                document.getElementById("message").innerHTML = "Ein Block, der bereits Teil der Mühle ist, kann nicht entfernt werden.";
            }
        }
    }
        ...
        if (numberOfTurns % 2 != 0) {
            ...
            if (checkMill(X, Y, 2)) {
                ...
                document.getElementById("message").innerHTML = "Eine Mühle wird gebildet. Klicke auf den grünen Block, um ihn zu entfernen.";
            } else {
                document.getElementById("message").innerHTML = "Klicke auf eine leere Stelle, um dein Stück zu platzieren.";
            }
            ...
            if (checkMill(X, Y, 1)) {
                isMillGreen = true;
                document.getElementById("turn").innerHTML = "P1";
                document.getElementById("message").innerHTML = "Eine Mühle wird gebildet. Klicke auf den roten Block, um ihn zu entfernen.";
            } else {
                document.getElementById("message").innerHTML = "Klicke auf eine leere Stelle, um dein Stück zu platzieren.";
            }
        ...
        if (numberOfTurns == 17) {
            document.getElementById("message").innerHTML = "Bewegen Sie nun einen Schritt, indem Sie auf Block klicken.";
        }
        numberOfTurns++;
        ...
        if (numberOfTurns % 2 != 0 && positionMatrix[X][Y] == 2) {
            ...
            if (checkThreeLeft(playerTwoCode)) {
                isRedThreeLeft = true;
                document.getElementById("message").innerHTML = "Rot kann sich jetzt überall bewegen (3 sind nur noch übrig)";
            } else {
                document.getElementById("message").innerHTML = "Bewegen Sie einen Schritt, indem Sie auf Block klicken";
            }
            ...
            if (checkThreeLeft(playerOneCode)) {
                ...
                document.getElementById("message").innerHTML = "Grün kann sich jetzt überall bewegen (3 sind nur noch übrig)";
            }
            else {
                document.getElementById("message").innerHTML = "Bewegen Sie einen Schritt, indem Sie auf Block klicken";
            }
...
function drawBlock(x, y, X, Y) {
    ...
    if (isActiveRed) {
        ...
        if (checkMill(X, Y, 2)) {
            isMillRed = true;
            document.getElementById("message").innerHTML = "Klicke auf den grünen Block, um ihn zu entfernen.";
        }
    } else {
        positionMatrix[X][Y] = 1;
        context.fillStyle = '#2E7D32';
        if (checkMill(X, Y, 1)) {
            isMillGreen = true;
            document.getElementById("message").innerHTML = "Klicke auf den roten Block, um ihn zu entfernen.";
        }
    }
    context.fill();
    context.lineWidth = 2;
...
function checkGameOver() {
    ...
    if (numberOfTurns >= 18) {
        if (redBlocks < 3 || greenBlocks < 3) {
            alert("Nür noch 2 " + ((greenBlocks < 3) ? "Grüne" : "Rote") + " Blöcke übrig !\n" +
                "Daher gewinnt spieler " + ((greenBlocks < 3) ? 2 : 1));
            location.reload(true);
        }
        else {
            //Check if no adjacent element available for any of the player.
            if (!canMove(playerOneCode, greenBlocks)) {
                alert("Keine möglichen Züge für Spieler " + playerOneCode + "übrig \n" +
                    "Daher gewinnt spieler " + playerTwoCode);
                location.reload(true);
            } else if (!canMove(playerTwoCode, redBlocks)) {
                alert("Keine möglichen Züge für Spieler " + playerTwoCode + "übrig \n" +
                    "Daher gewinnt spieler " + playerOneCode);
                location.reload(true);
            }
        }
